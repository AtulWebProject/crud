...
class Editjob implements ShouldQueue
{
    ...
    protected $storedata;
     ...
     protected $tag;
     protected $id;
    public function __construct($storedata,$imageurl,$description,$comment,$model,$longitude,$latitude,$copyright,$artist,$organation,$docname,$programname,$cameramaker,$rating,$tag,$id)
    {
      ...
      function convertDecimalToDMS($degree)
{
    ...
    foreach($allstoredata as $s){
        $allstoredata=$s[$d];
        // $sc=$allstoredata['store_code'];
        $s_code .=implode(',', [$allstoredata['store_code'].',']);
        $storecode[]=$allstoredata['store_code'];
    $f=$d.'.jpg';
 ...
 // test
    ...
    $res = Testseo::where('id', $this->id)->first();
    $d=$res['store_code'];
    $a=explode(',', $d);
    $result = array_diff($storecode, $a); 
    $updated_storeCode=array_merge($a,$result); 
    $c=implode(',', $updated_storeCode);
     // dd($c);
         $res->store_code=$c;
         $res->image=$imageurl;
